...
app.post("/attestations", async (req, res) => {
  ...
  // Generate UUID server-side instead of requiring it from client
  ...
  // Add the generated completion_id to the data object
  data.completion_id = completionId;

  const filename = `${completionId}.json`;
  const filePath = path.join(ATTESTATIONS_DIR, filename);

  // This check is now redundant with UUIDs but kept as safety measure
  ...
  try {
    ...
    await updateIndex(completionId, data);
    console.log(`✅ Saved attestation: ${completionId}`);
    res.json({ status: "saved", completion_id: completionId });
  } catch (err) {
