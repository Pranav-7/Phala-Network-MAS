...
async function verifyIntelAttestation(hex) {
  ...
  console.log('Intel Attestation Response:', JSON.stringify(response.body, null, 2))
...
async function verifyNvidiaAttestation(nvidiaPayload) {
  ...
  console.log('NVIDIA Attestation Response:', JSON.stringify(response.body, null, 2));
  ...
  if (Array.isArray(body) && body.length > 0) {
    ...
    if (Array.isArray(firstElement) && firstElement[0] === 'JWT') {
      ...
      console.log('Decoded NVIDIA JWT Payload:', JSON.stringify(jwtPayload, null, 2));
